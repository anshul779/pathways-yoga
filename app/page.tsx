...
import { PATHWAYS, CATEGORIES, PathwayMeta } from "./data/pathways";
...
export default function MasterHubPage() {
  ...
  const categoryCounts = useMemo(() => {
    const counts: Record<string, number> = { all: PATHWAYS.length };
    PATHWAYS.forEach((p) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-slate-50 text-slate-900 flex flex-col font-sans">
      ...
      <section className="sticky top-16 z-30 bg-slate-50/95 backdrop-blur-md py-3 px-4 sm:px-6 lg:px-8 border-b border-slate-200/80">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4 overflow-x-auto no-scrollbar pb-1 sm:pb-0">
          ...
          <div className="hidden lg:flex items-center gap-2 text-xs text-slate-500">
            ...
            <span>Showing {filteredPathways.length} of {PATHWAYS.length} pathways</span>
          </div>
        </div>
      </section>
